...
def main(args):
    c_sep, csv_file, has_header = extract_args(args)
...
def extract_args(args):
    # extraction
    try:
        csv_file, lhs, rhs, c_sep, has_header = '', [], [], '', None
        opts, args = getopt.getopt(args, "c:r:l:s:h:")
        for opt, arg in opts:
            if opt == '-c':
                csv_file = arg
            elif opt == '-r':
    ...
    try:
        ...
        sys.exit(-1)
    return c_sep, csv_file, has_header
