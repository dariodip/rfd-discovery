...
class NaiveDominance:
    ...
    def check_dominance(self, y: tuple, dist) -> bool:
        ...
        if len(self.tuples_set) == 0:
            return True
        for x in list(self.tuples_set):
            diff = np.array(x) - np.array(y)
            if all(diff <= 0):  # X dominates Y
                return False
            elif all(diff >= 0):  # Y dominates X
                self.tuples_set.remove(x)
        return True

    def clean_tuple_set(self, rows_to_add: set) -> set:
        if len(rows_to_add) == 1:
            return rows_to_add
        list_rows = list(rows_to_add)
        for i in range(0, len(list_rows)):
            for j in range(i+1, len(list_rows)):
                diff = np.array(list_rows[i]) - np.array(list_rows[j])
                if all(diff >= 0):
                    if list_rows[j] in rows_to_add:
                        rows_to_add.remove(list_rows[j])
                elif all(diff <= 0):
                    if list_rows[i] in rows_to_add:
                        rows_to_add.remove(list_rows[i])
        return rows_to_add
